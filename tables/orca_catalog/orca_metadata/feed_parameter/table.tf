module "feed_parameter_table" {
  source = "../../../../modules/delta-table"

  catalog_name   = var.catalog_name
  schema_name    = var.metadata_schema_name
  table_name     = "feed_parameter"
  table_comment  = "Feed-level configuration parameters - stores key-value parameters that apply to entire feeds"
  enable_cdf     = var.enable_cdf_for_metadata
  primary_key    = ["feed_id", "parameter_name"]
  ...
  columns = [
    ...
    },
    {
      ...
      type     = "BOOLEAN"
      nullable = false
      comment  = "Whether this parameter is required for feed execution"
    },
    {
      name     = "parameter_scope"
      type     = "STRING"
      nullable = false
      comment  = "Scope of parameter usage (runtime, configuration, schedule)"
    },
    {
      name     = "created_date"
      type     = "TIMESTAMP"
